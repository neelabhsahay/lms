...
@router.get("/filters")
def get_filterd_host(filter_cb: HostFilter,
	             db: Session = Depends(get_db),
	             token: str = Depends(decodeJWT)):
    return getFilteredHosts(db, filter_cb)
...
#@router.get("/{host_id}/details/", response_model=HostDetailOut)
@router.get("/{host_id}/details/")
def get_detail_host(host_id: int,
	            db: Session = Depends(get_db),
	            token: str = Depends(decodeJWT)):
    """Get whole details a specific host by ID"""
    return getDetailHost(db, host_id=host_id)
 
@router.post("/")
def create_host(host: HostCreate,
                db: Session = Depends(get_db),
                user: Session = Depends(getCurrentUsername),
                token: str = Depends(decodeJWT)):
    """Create a new host"""
...
@router.post("/details")
def create_detail_host(detail_host: HostDetailCreate,
	               db: Session = Depends(get_db),
                       user: Session = Depends(getCurrentUsername),
	               token: str = Depends(decodeJWT)):
    """Create a new host will all devices and their ports"""
    detail_host.updated_by = user
    return insertDetailHost(db, detail_host=detail_host)
