...
JWT_SECRET = config("secret", default='fallback_secret')
JWT_ALGORITHM = config("algorithm", default='HS256')
JWT_ISSUER = config("issuer", default='default_issuer_here')
...
def getCurrentUsername(Credentials: HTTPAuthorizationCredentials = Security(security)):
    credentials_exception = {
        "status_code": status.HTTP_401_UNAUTHORIZED,
        "detail": "Could not validate credentials",
        "headers": {"WWW-Authenticate": "Bearer"},
